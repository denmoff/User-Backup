...
def flight_check():
    ...
    if go1:
        print "The Destination is: %s and it appears to be available. The Computername is: %s" % (dest_path,comp_name)
    else:
        print "The Destination is: %s and it DOES NOT appear to be available. The Computername is: %s" % (dest_path,comp_name)
    ...
    if os.path.isfile(last_run_file):
        if int((time.time() - os.path.getmtime(last_run_file))/60) > run_interval:# has it run in the last n minutes
            ...
            go2 = False
            print "Backup has run within the last %s minutes" % run_interval
    else: # If Last Run File is missing, try to create it. If it fails, set go2 to False. If it succeeds, set to True.
        go2 = False
        print "Missing the last run file. Will attempt to create it..."
        try:
    ...
    if commands.getoutput('pgrep rsync') == '':
        ...
        go3 = False
        print "An instance of rsync is already running. Will try again shortly."

    # print "Backup directory exists: %s, a backup has not already run recently: %s, rsync is not currently running: %s" % (go1,go2,go3)
